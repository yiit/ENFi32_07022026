...
* **Auto Discovery topic**: The topic that should be used for sending the Discovery data. For Home assistant, this should be set to ``homeassistant/%devclass%/%unique_id%``, the available variables are the same as for the **Controller Publish** setting, and extended with these:
